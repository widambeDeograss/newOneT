...
export function loginRequest(email, password, pos, rememberMe = false, mfaCode = '') {
    const additional = {}
    if (mfaCode !== '') {
...
export function editPosTypeRequest(id, data) {
    return performAuthenticatedPutRequest('api/v1/pos/' + id + '?func=' + EDIT_POS_TYPE, data)
}
...
export function providerStatusRequest(id, status) {
    return performAuthenticatedPutRequest('api/v1/provider/' + id + '?s=' + status, {})
}

export function providerPutRequest(id, status) {
    return performAuthenticatedPutRequest('api/v1/provider/' + id + '?status=' + status, {})
}
...
export function ticketPutRequest(id, status) {
    return performAuthenticatedPutRequest('api/v1/ticket/' + id + '?status=' + status, {})
}
